...
label_prefix = '/data/mimic_data/files/mimic-cxr-jpg/2.0.0'
label_csv_file = "mimic-cxr-2.0.0-chexpert.csv"
label_csv_path = os.path.join(label_prefix, label_csv_file)
...
def parse_args():
    ...
    parser.add_argument("--output-file-name", required=False, default="debug.csv", help="output file.")
    parser.add_argument("--gpu-id", type=int, default=3, help="specify the gpu to load the model.")
    parser.add_argument("--debug", action="store_true", help="debug mode.")
    parser.add_argument("--use-images", action="store_true", help="use image embeddings.")
    parser.add_argument("--use-labels", action="store_true", help="use gt labels.")
...
def test_single_image(img_path, row, use_images=False, use_labels=False):
    ...
    if use_labels:
        ...
        labels = labels_df[(labels_df['subject_id'] == subject_id) & (labels_df['study_id'] == study_id)][LABEL_COLUMNS].values.tolist()
        # map labels to text 
        # chacha note: right now only deals with positive labels but can potentially extend to other cases
        label_text = []
        assert len(labels) == 1
        for i, label in enumerate(labels[0]):
            if label == 1:
                label_text.append(LABEL_COLUMNS[i])
        input_query = ', '.join(label_text)
        # input_query = 'Labels: '
    else:
...
with tqdm(total=total_generation_len) as pbar:
    ...
    for index, row in test_split.iterrows():
        ...
        output_text = test_single_image(img_path, row, use_images=args.use_images, use_labels=args.use_labels)
        if args.debug:
            print(output_text)
        # print(llm_message)
